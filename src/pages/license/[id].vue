  ...
  <VCard>
    <VCardTitle class="d-flex align-center justify-space-between">
      <div class="d-flex align-center gap-3">
        <span>Licença</span>
        <VChip :color="lic?.status === 'ACTIVE' ? 'success' : lic?.status === 'SUSPENDED' ? 'warning' : lic?.status === 'CANCELLED' ? 'error' : 'secondary'" class="text-uppercase" size="small">
{{ statusPt(lic?.status) }}
</VChip>
      </div>
    </VCardTitle>
...
<VCardTitle>Empresa</VCardTitle><VCardText>
          ...
          <VCol cols="12" md="6">
            <VCard variant="tonal">
<VCardTitle>Plano</VCardTitle><VCardText>
          ...
          <VCol cols="12" md="6">
            <VCard variant="tonal">
<VCardTitle>Datas</VCardTitle><VCardText>
              <div>Início: {{ formatDate(lic?.startDate) }}</div>
              <div>Expira: {{ formatDate(lic?.endDate) }}</div>
              <div>Pagamento: {{ formatDate(lic?.paymentDate) }}</div>
              <div>Criado: {{ formatDate(lic?.createdAt) }}</div>
              <div>Atualizado: {{ formatDate(lic?.updatedAt) }}</div>
            </VCardText>
</VCard>
          </VCol>
          <VCol cols="12" md="6">
          <VCard variant="tonal">
<VCardTitle>Consumo atual</VCardTitle><VCardText>
            <!--
